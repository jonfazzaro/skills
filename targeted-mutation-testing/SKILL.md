...
Do not use directory globs, whole-project mutation commands, unrelated tests, or a coverage percentage as a goal. Do not add mock-call assertions simply to kill a mutant; use [observable specifications](../references/observable-specifications.md).

Skip only a pure rename, formatting-only change, generated code, or a user-approved exception. Record the reason.

## Workflow

1. [Run the dedicated specifications first](../references/scoped-verification.md).
...
4. Classify every result:
   - **Killed**: record it.
   - **Actionable survivor**: add one [observable specification](../references/observable-specifications.md) for intended behavior, using [Nullable infrastructure wrappers](../references/nullable-infrastructure.md) for I/O.
...
6. Re-run the affected mutation range and the dedicated specifications. Run the [broader relevant suite](../references/scoped-verification.md) before committing.
...
- Testing private calls or mocks rather than [observable behavior](../references/observable-specifications.md).
